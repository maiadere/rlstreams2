...
HRESULT CloseAvi(HAVI avi) {
    ...
    if (au->as != 0) AVIStreamRelease(au->as);
    au->as = 0;
    if (au->psCompressed != 0) AVIStreamRelease(au->psCompressed);
    au->psCompressed = 0;
    if (au->ps != 0) AVIStreamRelease(au->ps);
    au->ps = 0;
    if (au->pfile != 0) AVIFileRelease(au->pfile);
    au->pfile = 0;
    AVIFileExit();
    delete au;
    return S_OK;
    ...
    if (au->psCompressed == 0)  // set the compression, prompting dialog if necessary
    {
        ...
        if (ShowDialog) {
            ...
            if (!res) {
                ...
                memcpy(opts, aopts[0], sizeof(AVICOMPRESSOPTIONS));
                au->iserr = true;
